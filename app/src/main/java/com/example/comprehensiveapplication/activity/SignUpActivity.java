...
import com.example.comprehensiveapplication.task.SignUpTask;
            ...
            case R.id.sign_up:
                ...
                if (getCheckBoxStatus()) {
                    ...
                    if (way == SIGN_UP_BY_PHONE) {
                        ...
                        signUpTask.execute(SIGN_UP, SIGN_UP_BY_PHONE, new Registrant(phone.getText().toString(), phoneVCode.getText().toString(), password.getText().toString()));
                    } else if (way == SIGN_UP_BY_EMAIL) {
                        registrant.setAccount(email.getText().toString());
                        registrant.setvCode(email.getText().toString());
                        signUpTask.execute(SIGN_UP, SIGN_UP_BY_EMAIL, new Registrant(email.getText().toString(), emailVCode.getText().toString(), password.getText().toString()));
                    }
                }
                break;
            case R.id.send_sms_v_code:
                signUpTask = new SignUpTask(signUpListener);
                signUpTask.execute(SEND, SEND_SMS, phone.getText().toString());
                break;
            case R.id.send_email_v_code:
                signUpTask = new SignUpTask(signUpListener);
                signUpTask.execute(SEND, SEND_EMAIL, email.getText().toString());
                break;
            default:
                break;
        }
